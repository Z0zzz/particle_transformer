...
def cut_pt_by_particle_type(pf_features, pf_points, pf_vectors, pf_mask, ptype = "hadron", k = 10):
    if ptype == "hadron":
        feature_idx = [6,7]
    elif ptype == "lepton":
        feature_idx = [9,10]
    ...
    keep_idx = []
    num_particles = pf_features.shape[0]
    for ipart in range(num_particles):
        for fidx in feature_idx:
            if pf_features[ipart, fidx] == 1:
                keep_idx.append(ipart)
                break
    
    if len(keep_idx) > k*2:
        keep_idx = keep_idx[:10] + keep_idx[-10:]
    ...
    for ipart in range(num_particles):
        ...
        for fidx in feature_idx:
            if pf_features[ipart, fidx] == 1:
                flag = False
            if pf_features[ipart, fidx] == 1 and ipart in keep_idx:
                new_pf_features.append(pf_features[ipart])
                new_pf_points.append(pf_points[ipart])
                new_pf_vectors.append(pf_vectors[ipart])
                new_pf_masks.append(pf_mask[ipart])
                break
    ...
    num_val_particles = len(new_pf_features)
    remainder = 128 - num_val_particles
    for i in range(remainder):
        new_pf_features.append([0.]*pf_features.shape[-1])
        new_pf_points.append([0.]*pf_points.shape[-1])
        new_pf_vectors.append([0.]*pf_vectors.shape[-1])
        new_pf_masks.append([0.]*pf_mask.shape[-1])
        
    new_pf_features = np.array(new_pf_features)
    new_pf_points = np.array(new_pf_points)
    new_pf_vectors = np.array(new_pf_vectors)
    new_pf_masks = np.array(new_pf_masks)
    
    return {"pf_features":new_pf_features, "pf_points":new_pf_points, "pf_vectors":new_pf_vectors, "pf_mask":new_pf_masks}
...
class JetClassDataset(Dataset):
    def __init__(self, data, labels, device = "cpu"):
        """
        Args:
            data (list or numpy array): Your input data.
            labels (list or numpy array): Corresponding labels for the data.
        """
        self.data = data
        self.labels = labels
    ...
    def __len__(self):
        ...
        return self.data["pf_features"].shape[0]

    def __getitem__(self, idx):
        # Get data and label at index `idx`
        features = torch.from_numpy(self.data["pf_features"][idx]).to(device)
        points = torch.from_numpy(self.data["pf_points"][idx]).to(device)
        vectors = torch.from_numpy(self.data["pf_vectors"][idx]).to(device)
        masks = torch.from_numpy(self.data["pf_mask"][idx]).to(device)
        
        label = self.labels[idx]
        
        return features, points, vectors, masks, label
...
if __name__ == "__main__":
    ...
    pf_mask = data["pf_mask"]
    pf_points = data["pf_points"]
    labels = data["label"]
    
    # process with mask
    k = 10
    num_events = pf_features.shape[0]
    num_particles = 128

    processed_features = np.ndarray([num_events, pf_features.shape[1], num_particles])
    processed_vectors = np.ndarray([num_events, pf_vectors.shape[1], num_particles])
    ...
    for iev in range(pf_features.shape[0]):
        ...
        processed_features[iev] = processed_event["pf_features"].T
    ...
    for batch in loader:
        ...
        with torch.no_grad():
            ...
            if num_data_processed > 10_000:
                break
    print("cut dataset accuracy: ", sum(torch.argmax(y_preds, axis=1) == torch.argmax(y_truth, axis=1))/y_truth.shape[0], flush=True)
